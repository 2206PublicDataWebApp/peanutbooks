            ...
            <div class="board_list">
	            ...
	            <article id="page-area">
					<c:if test="${currentPage != 1 }">
						<span class="prev"> 
							<a href="/admin/${urlVal }.kh?page=${currentPage - 1 }&qnaCategory=${qnaCategory}"> < </a>
						</span>
					</c:if>
					<c:forEach var="p" begin="${startNavi }" end="${endNavi }">
						...
						<c:if test="${currentPage ne p }">
							<span class="pages"> <a href="/admin/${urlVal }.kh?page=${p }&qnaCategory=${qnaCategory}">${p }</a></span>
						</c:if>
					</c:forEach>
					<c:if test="${maxPage > currentPage }">
						<span class="next"> <a href="/admin/${urlVal }.kh?page=${currentPage + 1 }&qnaCategory=${qnaCategory}"> > </a></span>
					</c:if>
				</article>
				<!-- 페이징 영역 종료 -->
			</div>
