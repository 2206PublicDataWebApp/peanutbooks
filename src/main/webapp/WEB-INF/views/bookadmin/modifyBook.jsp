...
<body onload = "loadTitle();">
	...
	<main>
		<form action="/book/ModifyNorBook.do" method="post" enctype="multipart/form-data">
			<section class="container">
				<article>
					<h1>도서등록</h1>
					<div id="title-area">
						<div class="title-text">
							<input type="hidden" id="hidden-title" value="${nBook.bookTitle }">
							<input type="hidden" id="hidden-cover" name ="coverRename" value="${nBook.coverRename }">
							<input type="hidden" id="bookNo" name ="bookNo" value="${nBook.bookNo }">
							<input type="hidden" id="hidden-writer" name ="" value="${nBook.writer }">
				...
				<article class="info-area row">
					<div class="col-md-6" id="pic-area">
						...
						<div class="align-items-center">
							<div class="align-items-center">
								<img class="headerimg" src="/resources/bookImg/${nBook.coverRename }"> <label
					...
					<div class="col-md-6">
						...
						<select name="category" id="" required="required">
							...
							<c:if test="${nBook.category == 'detective'}">selected="selected"</c:if>
							>추리</option>
							<option value="history"
							<c:if test="${nBook.category == 'history'}">selected="selected"</c:if>
							>역사</option>
							<option value="romance"
							<c:if test="${nBook.category == 'romance'}">selected="selected"</c:if>
							>로맨스</option>
							<option value="tale"
							<c:if test="${nBook.category == 'tale'}">selected="selected"</c:if>
							>동화</option>
							<option value="poem"
							<c:if test="${nBook.category == 'poem'}">selected="selected"</c:if>
							>시</option>
							<option value="novel"
							<c:if test="${nBook.category == 'novel'}">selected="selected"</c:if>
							>일반소설</option>
							<option value="other"
							<c:if test="${nBook.category == 'other'}">selected="selected"</c:if>
							>기타</option>
						</select> 
						
						<select name="language" id=""  required="required">
							<option value="">언어를 선택하세요</option>
							<option value="ko"
							<c:if test="${nBook.language == 'ko'}">selected="selected"</c:if>
							>한국어</option>
							<option value="en"
							<c:if test="${nBook.language == 'en'}">selected="selected"</c:if>
							>영어</option>
							<option value="ja"
							<c:if test="${nBook.language == 'ja'}">selected="selected"</c:if>
							>일본어</option>
						</select>
						</div>
						<h5>작품소개</h5>
				...
				<article id="first-contents">
					...
					<div class="contents-button row">
						...
						<button type="button" class="col-6" onclick="location.href='/book/norBookInfo?bookNo=${nBook.bookNo}'">취소</button>
					</div>
...
<script src="/resources/js/book/bookModify-2.js"></script>
